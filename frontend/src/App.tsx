...
function App() {
  ...
  // Agent Configuration State - Randomized Defaults
  // Ensure distinct profiles for interesting debate
  const initialPropProfile = getRandomItem(profilesData);
  const initialOppProfile = getRandomItem(profilesData.filter(p => p.Movement !== initialPropProfile.Movement));

  const [proponentProfile, setProponentProfile] = useState(initialPropProfile.Movement);
  const [proponentTone, setProponentTone] = useState(getRandomItem(tonesData).tone);
  const [proponentLanguage, setProponentLanguage] = useState(getRandomItem(languagesData).name);

  const [opponentProfile, setOpponentProfile] = useState(initialOppProfile.Movement);
  const [opponentTone, setOpponentTone] = useState(getRandomItem(tonesData).tone);
  const [opponentLanguage, setOpponentLanguage] = useState(getRandomItem(languagesData).name);
            ...
            <div className="mb-8 bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-gray-700 rounded-xl p-6">
              ...
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 bg-slate-50 dark:bg-[#0F172A] border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
