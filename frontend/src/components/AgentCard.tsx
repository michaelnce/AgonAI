...
export const AgentCard: React.FC<AgentCardProps> = ({ 
  ...
  return (
    <div className="bg-[#1E293B] border border-gray-700 rounded-xl p-5 flex-1 min-w-[300px] flex flex-col">
      ...
      {/* Info Card */}
      <div className="mt-auto bg-[#0F172A] rounded-lg p-3 text-xs space-y-2 border border-gray-800">
        {activeProfile && (
          <div>
            <span className="font-bold text-gray-400 block mb-1">PHILOSOPHY</span>
            <p className="text-gray-300 leading-relaxed">{activeProfile.Definition}</p>
            <div className="mt-2 flex gap-2 text-[10px]">
               <span className="text-gray-500">Root Conflict:</span>
               <span className="text-gray-300 italic">{activeProfile.RootConflict}</span>
            </div>
          </div>
        )}
        {activeTone && (
          <div className="pt-2 border-t border-gray-800">
            <span className="font-bold text-gray-400 block mb-1">TONE</span>
            <p className="text-gray-300">{activeTone.description}</p>
          </div>
        )}
      </div>
    </div>
