...
export const Sidebar: React.FC = () => {
  return (
    <aside className="w-64 bg-[#0F172A] text-gray-300 flex flex-col border-r border-gray-800">
      {/* User Info / Active Session */}
      <div className="p-4 border-b border-gray-800 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-gray-700"></div>
        <div>
          <h3 className="text-sm font-bold text-white">Debate History</h3>
...
const NavItem: React.FC<{ icon: string; label: string; active?: boolean }> = ({ icon, label, active }) => (
  <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
    active ? 'bg-blue-600/20 text-blue-400 border-l-2 border-blue-500' : 'hover:bg-gray-800'
  }`}>
    <span>{icon}</span>
